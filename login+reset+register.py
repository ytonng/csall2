...
class ForgetPasswordWindow(QtWidgets.QMainWindow):
    ...
    def save(self):
        verificationcode = self.ui.verificationEntry.text().strip()  # Remove leading/trailing spaces
        newpassword = self.ui.newpasswordEntry.text()
        confirmpassword = self.ui.confirmpasswordEntry.text()
        if not verificationcode or not newpassword or not confirmpassword:
            self.ui.errorMessageLabel.setText("Please fill in all fields.")
            return
        if verificationcode == self.verification_code:
            ...
            self.ui.errorMessageLabel.setText("Invalid verification code.")
